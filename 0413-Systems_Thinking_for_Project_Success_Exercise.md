...
> Example Fuzzy Cognitive Map representation of a coupled social and ecological system

In class today we are going apply some methods from participatory systems modeling to understand the drivers and dynamics of project success. We will begin with an introduction to systems thinking and a participatory complex systems modeling approach known as Fuzzy Cognitive Mapping (FCM). As a group, we will then define a set of model components affecting, and affected by, project success. Next, we will break into teams to build team-level models. We will use these models to run scenarios with the goal of building intuition and facilitating communication and learning among team members.  We will then report findings back to the class and discuss.   
...
This, of course, is wrong. The truth is far messier, and, anyway, we will not be privvy to it anytime soon. Today, however, we can be content in our attempt to move towards it. To do this, we need to trade our Newtonian, linear thinking for that of Donella Meadows, a nonlinear, systems-orientated approach (Meadows, 2008).    
...
Fuzzy Cognitive Maps (FCMs) combine aspects of dynamical systems, graph neural networks, and fuzzy logic to model and simulate complex systems generated by potentially heterogeneous phenomena. An FCM is a subjective representation of internal knowledge and its structure (Jones et al., 2011).  Such models encode a deterministic structure that is believed to emulate some approximation of human thought, and thereby simulate the human process of causal inference (Jones et al., 2011). Unlike classical causal inference that relies on DAGs (directed acyclic graphs) and particular statistical assumptions, FCM allows for more complex conceptions of causality via weighted and directed graphs whose topological structure may include cycles, thereby encoding feedback mechanisms that facilitate reasoning via systems thinking.  The first participatory application of FCM investigated decision-making processes of army officers engaged in complex war games (Klein & Cooper, 1982).  With respect to the wicked problems paradigm (Rittel & Weber, 1973) we met early in the semester, mental modeling excels in its ability to represent diverse sets of knowledge (Biggs et al., 2011) that may be aggregated into a collective cognitive map (Gray, 2016).  The methodology of forming a collective map is believed to increase stakeholder knowledge of a system and facilitate social learning (Papageorgiou, Markinos, & Gemptos, 2009; Gray, 2016)
...
&#9989; **<font color=red>DO THIS:</font>** Take 3 minutes and write a list of **concepts that affect *project success***. These components *must* be able to increase and decrease. Think of them as stocks of fish in a lake: The fish population can grow and shrink as long as it exists.  Use the nodes of this example FCM diagram for inspiration (note for later that, in this visualization, blue arrows are positive correlations and orange are negative):
...
&#9989; **<font color=red>DO THIS:</font>** Share ideas as a class. We will discuss and refine these as we have during previous classes.
...
### Team Roles
Teams will enter breakout rooms. When your team arrives, you will assign roles defined as follows.  Please note that everyone will participate in the activity, and that these roles may be shared among members if necessary. 
1. **Facilitator**: The facilitator guides the model generating process. They will:
    ...
    - "By how much: Slightly, moderately, for greatly?"


2. **Modeler**: The modeler's job is to translate the results of the discussion into Mental Modeler. This includes:

    a. Navigating to the [Mental Modeler website](https://www.mentalmodeler.com/#download) and login using the provided information
    
    b. Adding concepts to the cognitive map GUI; and,
    
    c. Assigning directional relations and weights using the GUI sliders; and
    
    d. Saving a screenshot of the diagram, downloading and saving the ```.mmp``` file, and exporting to CSV

All members are known as stakeholders or participants. 
...
&#9989; **<font color=red>DO THIS:</font>** Coordinate a 5 minute (max) presentation, and be ready to share what your entire team did with the instructors and class. We will have presentations at the end of next Wednesday.
